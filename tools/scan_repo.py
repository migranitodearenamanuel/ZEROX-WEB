...
# --- CONFIGURATION ---
DEFAULT_ROOT = "../zerox-core" 
DEFAULT_OUT = "../assets/knowledge"
...
def main():
    ...
    print("Loading NLP Model (all-MiniLM-L6-v2) for Semantic Fine-Tuning...")
    ...
    print(f"Generating Embeddings for {len(chunks_es)} text chunks...")
    corpus = [doc['text'] for doc in chunks_es]
    embeddings = model.encode(corpus)
    
    # Store vectors mapping
    vectors = {}
    for i, emb in enumerate(embeddings):
        vectors[i] = emb.tolist() # Convert numpy to list for JSON
    ...
    with open(f"{output_dir}/vectors.json", 'w', encoding='utf-8') as f:
        json.dump(vectors, f)
        
    with open(f"{output_dir}/sources.json", 'w', encoding='utf-8') as f:
        json.dump(sources, f, indent=2)
        
    print(f"DONE. Indexed {total_files} files and generated semantic vectors.")
